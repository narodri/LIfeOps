...
export const cards = sqliteTable("cards", {
  id: text("id").primaryKey(), title: text("title").notNull(), theme: text("theme").notNull(), status: text("status").notNull(), period: text("period").notNull(),
  deadline: text("deadline"), detailMemo: text("detail_memo"), createdAt: integer("created_at").notNull(), updatedAt: integer("updated_at").notNull(), lastProgressMemoAt: integer("last_progress_memo_at")
});
export const progressMemos = sqliteTable("progress_memos", {id:text("id").primaryKey(), cardId:text("card_id").notNull(), body:text("body").notNull(), createdAt:integer("created_at").notNull()});
