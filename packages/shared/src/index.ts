...
export function staleLevel(lastProgressMemoAt?: string | null, now = new Date()): StaleLevel {
  if (!lastProgressMemoAt) return "strong";
  const diffDays = Math.floor((now.getTime() - new Date(lastProgressMemoAt).getTime()) / 86400000);
